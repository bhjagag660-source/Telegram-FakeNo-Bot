...
def market(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🛡️ Güvence Kanalı", url=GUVENCE_BALANTISI)
    )
...
# --- DİĞER KISIMLAR (SATIN ALMA VE ADMİN) ---
...
def adm_calls(call):
    ...
    if call.data == "adm_stats":
        ...
        m = bot.send_message(call.message.chat.id, "ID MİKTAR yaz (Örn: 8434939976 10):")
...
def jeton_islem(message):
    ...
    except: bot.send_message(message.chat.id, "❌ Hata! Tekrar dene.")

def duyuru_gonder(message):
    if message.from_user.id not in ADMIN_LIST: return
    with sqlite3.connect('bot_verisi.db', check_same_thread=False) as conn: users = conn.execute("SELECT user_id FROM users").fetchall()
    for u in users:
        try: bot.send_message(u[0], f"📣 **DUYURU**\n\n{message.text}")
        except: pass
    bot.send_message(message.chat.id, "✅ Duyuru gönderildi.")
